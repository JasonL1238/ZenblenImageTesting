...
disjoint chunks) — same policy as ``detect_spill``, never keeps only the
single highest-confidence instance the way ``get_yolo_roi`` does for the
...
def detect_chunk_yolo(image: np.ndarray, config: Config) -> np.ndarray:
    ...
    catches this and returns an empty mask."""
    h, w = image.shape[:2]
