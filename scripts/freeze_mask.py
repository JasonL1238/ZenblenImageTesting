...
REF = ROOT / "data/images/UserGrab_c5032014-b3ad-4988-b35c-3fa78cdfa170_2026_06_16_18_40_13.jpg"
OUT = ROOT / "outputs/frozen_mask_test"
OUT.mkdir(parents=True, exist_ok=True)

# --- 1. compute + save the frozen mask from the reference image ---
ref_img = cv2.imread(str(REF))
frozen_mask, frozen_bbox = detect_container(ref_img)
ref_h, ref_w = frozen_mask.shape[:2]
cv2.imwrite(str(ROOT / "outputs/frozen_mask.png"), frozen_mask)
print(f"Frozen mask from {REF.name}: bbox={frozen_bbox}, resolution={ref_w}x{ref_h}")
print(f"Saved -> outputs/frozen_mask.png\n")
