"""Seed candidate chunk polygons from YOLO-chunk (classical fallback).
...
clip to ROI; classical only if chunk weights fail. Annotators correct the
model's guess in app_multi (mode 4) instead of free-drawing from scratch.
...
DEFAULT_OVERLAYS = REPO / "outputs" / "chunk_autolabel_review"
...
def main() -> None:
    ...
    db.ensure_dirs()
    cfg = Config()
    # Force YOLO-chunk primary for seeds (classical only if weights fail)
    cfg.chunk_detector_priority = ["yolo", "classical"]
