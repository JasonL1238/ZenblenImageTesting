...
``mode_status`` tables — the legacy ``labels`` table is never
modified here.
...
Run (no torch needed — free-draw; YOLO chunk seeds load when present
...
def api_chunk_seed(file_id: int):
    """Optional YOLO candidate shapes (chunk mode seed).
